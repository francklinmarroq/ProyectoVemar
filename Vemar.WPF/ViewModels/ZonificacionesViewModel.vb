...
Public Class ZonificacionesViewModel : Inherits ViewModelBase : Implements INotifyPropertyChanged
    Private ReadOnly _service As IDataService(Of Zonificacion)
    Private _items As New ObservableCollection(Of Zonificacion)
    ...
    Public Property Items As ObservableCollection(Of Zonificacion)
        Get
            Return _items
        End Get
        Set(v As ObservableCollection(Of Zonificacion))
            _items = v
    ...
    Private _editando As Zonificacion = Nothing
    ...
    Public Sub New(service As IDataService(Of Zonificacion))
        _service = service
        AgregarCommand = New RelayCommand(Sub(o)
                                             _editando = Nothing
                                             Nombre = ""
                                             Dim win As New AgregarZonificacionWindow()
                                             win.DataContext = Me
                                             win.Owner = Application.Current.MainWindow
                                             win.ShowDialog()
                                         End Sub)
        ModificarCommand = New RelayCommand(Sub(o)
                                               Dim item = TryCast(o, Zonificacion)
                                               If item Is Nothing Then Return
                                               _editando = item
                                               Nombre = item.Zonificacion
                                               Dim win As New AgregarZonificacionWindow()
                                               win.DataContext = Me
                                               win.Owner = Application.Current.MainWindow
                                               win.ShowDialog()
    ...
    Public Async Sub CargarItems()
        Try
            Items = New ObservableCollection(Of Zonificacion)(Await _service.GetAll())
    ...
    Public Async Sub Eliminar(obj As Object)
        Dim item = TryCast(obj, Zonificacion)
        If item Is Nothing Then Return
        If MessageBox.Show($"¿Eliminar ""{item.Zonificacion}""?", "Confirmar", MessageBoxButton.YesNo, MessageBoxImage.Warning) = MessageBoxResult.Yes Then
            Try
    ...
    Public Async Sub Guardar(obj As Object)
        Try
            Dim item As New Zonificacion With {.Zonificacion = Nombre}
